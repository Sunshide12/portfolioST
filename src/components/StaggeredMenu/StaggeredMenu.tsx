...
export const StaggeredMenu = ({
  position = 'right',
  colors = ['var(--color-border)', 'var(--color-primary)'],
  items = [],
  socialItems = [],
  displaySocials = true,
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      let preLayers: any[] = [];
      if (preContainer) {
        preLayers = Array.from((preContainer as any).querySelectorAll('.sm-prelayer'));
      }
  ...
  return (
    ...
    >
      <div className="sm-overlay" onClick={closeMenu} aria-hidden="true" />

      <div ref={preLayersRef} className="sm-prelayers" aria-hidden="true">
        {(() => {
          const raw = colors && colors.length ? colors.slice(0, 4) : ['var(--color-border)', 'var(--color-primary)'];
          let arr = [...raw];
          if (arr.length >= 3) {
      ...
      <button
        ref={toggleBtnRef}
        className="sm-toggle topbar__hamburger"
                ...
                <li className="sm-panel-itemWrap" key={it.label + idx}>
                  <a
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      closeMenu();
                      if (onNavigate) onNavigate(it.link);
                    }}
                  >
